...
namespace BudgetBadger.macOS
{
    ...
    public class InAppBillingImplementation : IInAppBilling
    {
        ...
        public Task<bool> ConnectAsync(ItemType itemType = ItemType.InAppPurchase) => Task.FromResult(true);
        ...
        public async Task<IEnumerable<InAppBillingProduct>> GetProductInfoAsync(ItemType itemType, params string[] productIds)
        {
            ...
            return products.Select(p => new InAppBillingProduct
            {
                ...
                LocalizedIntroductoryPrice = p.IntroductoryPrice?.LocalizedPrice() ?? string.Empty,
                MicrosIntroductoryPrice =  (long)((p.IntroductoryPrice?.Price?.DoubleValue ?? 0) * 1000000d)
            });
        }
        ...
        async Task<SKPaymentTransaction> PurchaseAsync(string productId)
        {
            ...
            var product = await GetProductAsync(new string[] { productId });

            var payment = SKPayment.CreateFrom(product.FirstOrDefault());
        ...
        public Task<bool> FinishTransaction(InAppBillingPurchase purchase) =>
    ...
    class PaymentObserver : SKPaymentTransactionObserver
    {
        ...
        public bool ShouldAddStorePayment(SKPaymentQueue queue, SKPayment payment, SKProduct product)
        {
